...
// Override of the built-in watch task by this custom one. Required to use this task on windows where fs.watch
// must be used. Done here to test things out as it's not coming without some issue, namely on unwatchFile
// which does't seem to properly "unwatch" when fs.watch was used, but works good when fs.watchFile was used.
//
// To avoid this problem which might raise an EMFILE error, we prevent the watch of files
// that were previously "watched" (and stored in the module-scopped watched array).

var watched = [];

// set as watchit instead of raw override. Needs further work, that approach of
// watched array of files and watch prevents do not work quite welll.
task.registerTask('watchit', 'Run predefined tasks whenever watched files change.', function(prop) {
  ...
  files.forEach(function(filepath) {
    // prevent if this file is already "watched"
    if(!!~watched.indexOf(filepath)) return;

    // add to the watched array otherwise.. and watch
    watched.push(filepath);
    fs.watch(filepath, function(ev, filename) {
      if(!filename) return;
      change(ev, filepath);
    });
  });
